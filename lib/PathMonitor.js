...
var ChildPathMonitor = require('./ChildPathMonitor');
require('colors');
...
PathMonitor.prototype = {
   ...
   _process: function(fn, snap) {
      var dat = snap.val(),
        key = snap.key;
      if( this.filter(dat) ) {
         fn.call(this, this.resolve(dat, key), this.parse(dat, key));
      }
   },
...
exports.process = function(esc, paths) {
  if( fbutil.isString(paths) ) {
    ...
    paths.forEach(function(pathProps) {
      if (pathProps.parent_path) {
        new NestedPathMonitor(fbutil.fbRef(pathProps.watch_path), function(parentKey) {
          pathProps.paths.forEach(function(childPath) {
            return new ChildPathMonitor(esc, Object.assign({}, pathProps, { path: childPath }), parentKey, pathProps.parent_path + '/' + parentKey);
          });
        });
      } else {
